...
$(function () {
  ...
  t.length &&
    (e = t.DataTable({
      ...
      columns: [{ data: "invoice_id" }, { data: "invoice_id" }, { data: "invoice_id" }, { data: "invoice_status" }, { data: "issued_date" }, { data: "client_name" }, { data: "total" }, { data: "balance" }, { data: "invoice_status" }, { data: "action" }],
      columnDefs: [
        {
          className: "control",
        ...
        },
        {
          ...
          render: function (e, t, a, n) {
            var s = a.invoice_status,
              i = a.due_date;
            return "<span class='d-inline-block' data-bs-toggle='tooltip' data-bs-html='true' title='<span>" + s + '<br> <span class="fw-medium">Balance:</span> ' + a.balance + '<br> <span class="fw-medium">Due Date:</span> ' + i + "</span>'>" + { Sent: '<span class="badge badge-center rounded-pill bg-label-secondary w-px-30 h-px-30 d-flex align-items-center justify-content-center"><i class="ti ti-circle-check ti-xs"></i></span>', Draft: '<span class="badge badge-center rounded-pill bg-label-primary w-px-30 h-px-30 d-flex align-items-center justify-content-center"><i class="ti ti-device-floppy ti-xs"></i></span>', "Past Due": '<span class="badge badge-center rounded-pill bg-label-danger w-px-30 h-px-30 d-flex align-items-center justify-content-center"><i class="ti ti-info-circle ti-xs"></i></span>', "Partial Payment": '<span class="badge badge-center rounded-pill bg-label-success w-px-30 h-px-30 d-flex align-items-center justify-content-center"><i class="ti ti-circle-half-2 ti-xs"></i></span>', Paid: '<span class="badge badge-center rounded-pill bg-label-warning w-px-30 h-px-30 d-flex align-items-center justify-content-center"><i class="ti ti-chart-pie ti-xs"></i></span>', Downloaded: '<span class="badge badge-center rounded-pill bg-label-info w-px-30 h-px-30 d-flex align-items-center justify-content-center"><i class="ti ti-arrow-down-circle ti-xs"></i></span>' }[s] + "</span>";
          },
        },
        {
          targets: 4,
          ...
          render: function (e, t, a, n) {
            ...
            return '<div class="d-flex justify-content-start align-items-center"><div class="avatar-wrapper"><div class="avatar avatar-sm me-3">' + (!0 === l ? '<img src="' + assetsPath + "img/avatars/" + (c + ".png") + '" alt="Avatar" class="rounded-circle">' : '<span class="avatar-initial rounded-circle bg-label-' + ["success", "danger", "warning", "info", "primary", "secondary"][Math.floor(6 * Math.random())] + '">' + (l = (((l = (s = a.client_name).match(/\b\w/g) || []).shift() || "") + (l.pop() || "")).toUpperCase()) + "</span>") + '</div></div><div class="d-flex flex-column"><a href="pages-profile-user.html" class="text-heading text-truncate"><span class="fw-medium">' + s + '</span></a><small class="text-truncate">' + i + "</small></div></div>";
          },
        },
        {
          targets: 5,
        ...
        },
        {
          ...
          render: function (e, t, a, n) {
            ...
            return '<span class="d-none">' + moment(a).format("YYYYMMDD") + "</span>" + moment(a).format("DD MMM YYYY");
          },
        },
        {
          targets: 7,
      ...
      initComplete: function () {
        this.api()
          ...
          .every(function () {
            var t = this,
              a = $('<select id="UserRole" class="form-select"><option value=""> Invoice Status </option></select>')
